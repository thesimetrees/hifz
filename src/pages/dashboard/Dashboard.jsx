...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const sinkron = async () => {
      ...
      const hasil = await Promise.all(daftar.map(async (p) => {
        if (!p.invoice || p.status === 'Lunas' || p.status === 'Dibatalkan') return p
        try {
          const server = await api(`/toko/orders/${p.invoice}`, { auth: false })
          if (server.status !== p.status) {
            berubah = true
            return { ...p, status: server.status }
          }
        } catch {
  ...
  // notifikasi: konfirmasi pembayaran + tagihan menunggu + penilaian baru dari guru
  const notifikasi = [
    ...notifBayar,
    ...
    <div className={`db db--user${collapsed ? ' is-collapsed' : ''}`}>
                  ...
                  <aside className="db-col-side">
                    ...
                    {tagihanTunggu.length > 0 && (
                      <section className="db-alert">
                        <div>
                          <strong>{tagihanTunggu[0].invoice} menunggu pembayaran</strong>
            ...
            {tab === 'program' && !programTerpilih && (
              <div className="db-page">
                <div className="db-kelas-atas">
                  <div>
                    <h4>Kelas saya</h4>
                    <p className="db-kelas-sub-info">{aktif.length} kelas berjalan · {selesai.length} kelas selesai</p>
                  </div>
                  ...
                  {programSaya.map((p) => (
                    ...
                    >
                      <div className="db-kelas-media">
                        ...
                        <em className={`db-kelas-pct${p.status === 'selesai' ? ' is-selesai' : ''}`}>
                      ...
                      <div className="db-kelas-isi">
                        ...
                        <div className="db-kelas-progres">
                          ...
                          <em>{p.modulSelesai}/{p.totalModul} modul</em>
                        </div>
                        <span className="db-kelas-lanjut">
                          {p.status === 'selesai' ? 'Lihat kelas' : p.modulSelesai > 0 ? 'Lanjutkan belajar' : 'Mulai belajar'}
                    ...
                    {programTerpilih.status === 'berjalan' ? (
                      ...
                      <section className="db-prog-done">
                        ...
                        <button type="button" className="db-btn">
                          <Download size={15} strokeWidth={1.9} /> Unduh sertifikat
                        </button>
                      </section>
                    )}
                  ...
                  <aside className="db-prog-side">
                    <section className="db-block">
                      ...
                      <div className="db-prog-btns">
                        <button type="button" className="db-btn db-btn--deep" onClick={() => setTab('rapot')}>
                          <Award size={15} strokeWidth={1.9} /> Sertifikat & Rapot
                        </button>
                        <button type="button" className="db-btn">
                          <MessagesSquare size={15} strokeWidth={1.9} /> Grup Kelas
                        </button>
                      </div>
                    </section>
                    ...
                    <section className="db-block">
                      ...
                      {subJalan && (hadir.includes(subJalan.id) ? (
                        <p className="db-absen-ok"><CheckCircle2 size={14} strokeWidth={2} /> Hadir · {subJalan.judul}</p>
                      ) : (
                        <p className="db-absen-note"><UserCheck size={14} strokeWidth={2} /> Kehadiran dicatat oleh guru.</p>
                      ))}
                    </section>
                    ...
                    {j && praw && (
                      <section className="db-block">
                        ...
                        <div className="db-belajar-sync db-belajar-sync--kolom">
                          <a className="db-btn db-btn--ghost db-btn--sm" href={linkGoogleKalender(praw, j)} target="_blank" rel="noreferrer">
                            <CalendarPlus size={14} strokeWidth={2} /> Google Calendar
                          </a>
                          <button type="button" className="db-btn db-btn--ghost db-btn--sm" onClick={() => unduhICS(praw, j)}>
                            <Download size={14} strokeWidth={2} /> Simpan .ics
                          </button>
                          <button type="button" className={`db-btn db-btn--sm${ingat ? '' : ' db-btn--ghost'}`} onClick={() => togglePengingat(praw)}>
                            <BellRing size={14} strokeWidth={2} /> {ingat ? 'Pengingat aktif' : 'Pengingat'}
                          </button>
                        </div>
                      </section>
                    )}
                  ...
                  <div className="db-table-head db-table-head--riwayat">
                    ...
                    <div className="db-riw-filter" role="tablist" aria-label="Filter riwayat">
                      ...
                      <button type="button" className={filterRiwayat === 'program' ? 'is-active is-program' : ''} onClick={() => setFilterRiwayat('program')}>
                        <GraduationCap size={13} strokeWidth={2} /> Program <i>{jumlahProgram}</i>
                      </button>
                      <button type="button" className={filterRiwayat === 'toko' ? 'is-active is-toko' : ''} onClick={() => setFilterRiwayat('toko')}>
                        <ShoppingCart size={13} strokeWidth={2} /> Toko <i>{jumlahToko}</i>
                      </button>
                    </div>
                    ...
                    <table className="db-table">
                      ...
                      <tbody>
                        {riwayatTampil.length === 0 && (
                          <tr>
                            <td colSpan={8} className="db-kosong">
                        ...
                        {riwayatTampil.map((r) => {
                          ...
                          <tr key={r.invoice} className={`db-row--${jenis}`}>
                            ...
                            <td>
                              <span className={`db-jenis db-jenis--${jenis}`}>
                                {jenis === 'program' ? <GraduationCap size={12} strokeWidth={2.1} /> : <ShoppingCart size={12} strokeWidth={2.1} />}
                                {jenis === 'program' ? 'Program' : 'Toko'}
                              </span>
                            </td>
                            <td>{r.tanggal}</td>
                            <td className="db-td-item">{r.item}</td>
                            ...
                            <td>
                              <span className={`db-status ${r.status === 'Lunas' ? 'is-lunas' : 'is-tunggu'}`}>
                                {r.status}
                              </span>
                            </td>
                            <td>
                              {r.status === 'Lunas' ? (
                                <button type="button" className="db-td-btn" title="Unduh invoice">
                                  <Download size={14} strokeWidth={1.9} />
                                </button>
                              ) : (
                                <button type="button" className="db-td-btn db-td-btn--bayar">Bayar</button>
                              )}
                            </td>
                          </tr>
                          )
      ...
      {bayarProgram && (
        <BayarModal
          ...
          item={bayarProgram.nama}
          total={bayarProgram.harga}
          penerima={user.nama}
          onTutup={() => setBayarProgram(null)}
          onSelesai={() => {
            ikutiProgram(user.email, bayarProgram.id)
